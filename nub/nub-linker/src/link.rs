...
//! 6. **Emit [`ProgramBlob`]** with the raw code bytes in
//!    [`ProgramBlob::code`], mapped read-only at the fixed `CODE_BASE`
//!    by the runtime. The recompiler consumes the raw bytes directly.

use crate::LinkError;
use crate::elf::parse_linked_elf;
use nub_program::abi::{CODE_BASE, DATA_BASE, PAGE_SIZE, SP_REG};
use nub_program::{Endpoint, ProgramBlob, Regions};
...
/// Link an RV ELF into a PVM2 [`ProgramBlob`]. [`ProgramBlob::code`]
/// holds the raw RV+C+custom-0 bytes, mapped read-only at [`CODE_BASE`]
/// by the runtime.
pub fn link_elf(elf_data: &[u8]) -> Result<ProgramBlob, LinkError> {
    ...
    for (&call_v, &target) in &elf.call_targets {
        let auipc_off = vaddr_to_offset(call_v).ok_or_else(|| {
            LinkError::InvalidSection(format!(
    ...
    // ---- 7. Region geometry + blob emission -------------------------
    //
    // Regions are page counts only; `Regions::iter()` derives the
    // placement, stacking stack/ro/rw/heap linearly from DATA_BASE.
    // `ProgramBlob::new` zero-extends the ro/rw buffers to whole pages
    // and enforces the geometry bounds (code below DATA_BASE, data
    // within the 4 GiB guest range).
    //
    // Code is mapped RO at the fixed `CODE_BASE` by the runtime — not
    // via a declarative mapping, so an untrusted program cannot
    // relocate it. Only data regions appear in the geometry.
    let regions = Regions {
        stack_pages: elf.stack_size / PAGE_SIZE,
        ro_pages: (ro_data_rewritten.len() as u32).div_ceil(PAGE_SIZE),
        rw_pages: (rw_data_rewritten.len() as u32).div_ceil(PAGE_SIZE),
        heap_pages: elf.heap_pages,
    };

    let stack_top = regions.stack_top();
    for def in endpoints.values_mut() {
...
fn expect_auipc(code: &[u8], off: usize, v: u64) -> Result<(), LinkError> {
    if off + 4 > code.len() {
        return Err(LinkError::InvalidSection(format!(
...
fn fold_auipc_to_lui(code: &mut [u8], off: usize, v: u64, eff: u32) -> Result<(), LinkError> {
    if off + 4 > code.len() {
        return Err(LinkError::InvalidSection(format!(
            "link_elf: AUIPC reloc at vaddr {v:#x} truncated by section end"
        )));
...
fn fixup_code_pcrel(
    ...
    let remap = |o: usize| -> Result<usize, LinkError> {
        offset_map.get(&o).copied().ok_or_else(|| {
            LinkError::InvalidSection(format!("fixup_code_pcrel: offset {o:#x} not in offset_map"))
        })
    };
...
///   backward compat").
fn rewrite_ecall_markers(code: &mut [u8]) -> Result<(), LinkError> {
    let n = code.len();
    let mut i = 0;
    while i + 2 <= n {
...
fn validate_pvm2(code: &[u8]) -> Result<(), LinkError> {
    let n = code.len();
    let mut i = 0;
    while i < n {
        if i + 2 > n {
        ...
        let check = |name: &str, r: u32| -> Result<(), LinkError> {
            if r == 3 || r == 4 || r >= 16 {
                return Err(LinkError::InvalidSection(format!(
...
fn read_endpoint_descriptors(
    ...
    for section_bytes in &sections {
        ...
        for chunk in section_bytes.chunks(DESCRIPTOR_SIZE) {
            ...
            // Third metadata byte is opaque to the linker; a personality
            // may interpret it (JAVM reads it as the arg-cnode size).
...
fn align_branch_targets(
    code: &mut Vec<u8>,
    extra_targets: &[usize],
) -> Result<BTreeMap<usize, usize>, LinkError> {
...
mod tests {
    ...
    fn validate_rejects_standard_ecall() {
        ...
        assert!(matches!(err, LinkError::InvalidSection(_)));
    }
    ...
    fn validate_rejects_producer_x3_use() {
        ...
        let err = validate_pvm2(&code).unwrap_err();
        let LinkError::InvalidSection(msg) = err else {
            panic!();
        };
