...
mod tests {
    ...
    fn image_cap_roundtrip_preserves_hash() {
        ...
        img.code = alloc::vec![0u8, 10u8, 42];
        img.packed_bitmask = alloc::vec![0b011u8];
        let mut endpoints = alloc::collections::BTreeMap::new();
        endpoints.insert(
            0u8,
            crate::image::EndpointDef {
                entry_pc: 1,
                arg_registers: 0,
                arg_cnode_size: 0,
                initial_regs: alloc::collections::BTreeMap::new(),
            },
        );
