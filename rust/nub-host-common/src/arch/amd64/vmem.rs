...
/// Page is Read/Write (if not set page is read only so long as the WP bit in CR0 is set to 1 (which the nub host sets))
...
mod tests {
    ...
    /// needed because the `GuestPageTableBuffer` lives in `nub-host-kvm`, which would cause a circular dependency
    struct MockTableOps {
