...
pub enum ExitReason {
    ...
    HostCall(u32),
    /// `ecall.jar` (custom-0 funct3=001, no immediate). The
    /// recompiler-side counterpart to the interpreter's
    /// EcallKind::Ecall routing. In the kernel, the integration layer
    /// reads φ\[11\] (mgmt op) and φ\[7\]/φ\[8\] (operands) to dispatch.
    Ecall,
}
