...
impl<T: Clone, const N: usize> FixedVector<T, N> {
...
impl<T, const N: usize> core::ops::Deref for FixedVector<T, N> {
    type Target = [T];
    fn deref(&self) -> &[T] {
        &self.inner
    }
...
impl<T: fmt::Debug, const N: usize> fmt::Debug for FixedVector<T, N> {
...
impl<T: Decode, const N: usize> Decode for FixedVector<T, N> {
    ...
    fn from_ssz_bytes(bytes: &[u8]) -> Result<Self, DecodeError> {
        let decoded = decode_fixed_vector::<T>(bytes, N)?;
        let mut inner: Vec<T> = Vec::with_capacity(N);
        for t in decoded {
            inner.push(t);
        }
        Ok(Self { inner })
...
impl<T: HashTreeRoot + Encode, const N: usize> HashTreeRoot for FixedVector<T, N> {
...
pub(crate) fn encode_fixed_vector<'a, T, I>(items: I, buf: &mut Vec<u8>)
where
    T: Encode + 'a,
    I: Iterator<Item = &'a T>,
{
...
pub(crate) fn decode_fixed_vector<T: Decode>(
    bytes: &[u8],
    n: usize,
    ...
    if T::is_ssz_fixed_len() {
        ...
        decode_var_collection::<T>(bytes, Some(n))
    }
}
...
/// offset (list mode).
pub(crate) fn decode_var_collection<T: Decode>(
    bytes: &[u8],
    ...
    for i in 0..n {
        ...
        out.push(T::from_ssz_bytes(slice)?);
    }
    Ok(out)
}
